...
const Search = ({ isOpen, onClose }) => {
  ...
  const { results, popularSearches, totalResults, hasResults } = useSearch(query);
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (selectedIndex < popularSearches.length && !query) {
          ...
          const resultIndex = query ? selectedIndex : selectedIndex - popularSearches.length;
  ...
  }, [handleResultClick, handlePopularSearch, selectedIndex, popularSearches, query, results, isOpen, onClose]);
                  ...
                  <>
                    {hasResults ? (
                          ...
                          <div key={category.category} className="mb-6 last:mb-0">
                            ...
                            <div>
                              {category.items.map((result, resultIndex) => {
                                const flatIndex = resultIndex + (categoryIndex === 0 ? 
                                  (query ? 0 : popularSearches.length) : 
                                  results.slice(0, categoryIndex).reduce((sum, cat) => sum + cat.items.length, 0) + (query ? 0 : popularSearches.length)
                                );
                      ...
                      <div className="p-12 text-center">
                        ...
                        <p className="text-ink-muted">Try different keywords or browse our popular searches</p>
                      </div>
                    )}
                  </>
                )}
